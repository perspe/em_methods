...
class SMBase:
    ...
    Has the necessary properties defined
    """
    ...
    @property
    def S11(self) -> npt.NDArray:
        return self._S11

    @property
    def S12(self) -> npt.NDArray:
        return self._S12
...
class SFree(SMBase):
    ...
    def __init__(self, Kx: spmatrix, Ky: spmatrix) -> None:
        ...
        Q: spmatrix = scs.bmat([[KxKy, eye - Kx2], [Ky2 - eye, -KxKy]], **spm_format)
...
class SRef(SMBase):
    ...
    def __init__(
        ...
        Kx2 = Kx @ Kx
        Ky2 = Ky @ Ky
        matrix: spmatrix = scs.bmat(
            [[KxKy, e_ref * u_ref * eye - Kx2], [Ky2 - e_ref * u_ref * eye, -KxKy]],
            **spm_format,
        )
        Q = matrix / u_ref
        logger.debug(f"\nQ:\n{Q}")
        self._W: spmatrix = scs.block_diag((eye, eye), **spm_format)
        eig_matrix: spmatrix = scs.block_diag(
            (-1j * Kz_ref, -1j * Kz_ref), **spm_format
        )
...
class STrn(SMBase):
    ...
    def __init__(
        ...
        Ky2 = Ky @ Ky
        matrix = scs.bmat(
            [[KxKy, e_trn * u_trn * eye - Kx2], [Ky2 - e_trn * u_trn * eye, -KxKy]],
            **spm_format,
        )
        Q: spmatrix = matrix / u_trn
        self._W: spmatrix = scs.block_diag((eye, eye), **spm_format)
        eig_matrix: spmatrix = scs.block_diag((1j * Kz_trn, 1j * Kz_trn), **spm_format)
...
class SMatrix(SMBase):
    ...
    def __init__(
        ...
        inv_er = scs.csc_matrix(inv(er, check_finite=False), dtype=np.complex64)
        inv_u0 = scs.csc_matrix(inv(u0, check_finite=False), dtype=np.complex64)
        logger.debug(
            f"""
inv_er ({inv_er.shape}):
{inv_er}
...
"""
        ...
        inv_u0Ky = inv_u0 @ Ky
        P: spmatrix = scs.bmat(
            [[Kx @ inv_erKy, u0 - Kx @ inv_erKx], [Ky @ inv_erKy - u0, -Ky @ inv_erKx]],
            **spm_format,
        )
        Q: spmatrix = scs.bmat(
            [[Kx @ inv_u0Ky, er - Kx @ inv_u0Kx], [Ky @ inv_u0Ky - er, -Ky @ inv_u0Kx]],
            **spm_format,
        )
...
"""
        ...
        self._W = P @ Q
        logger.debug(f"Omega2({self._W.shape}::{self._W.count_nonzero()})\n{self._W}")
        logger.info("Solve for eigenvalues")
        eigs, self._W = eig(
            np.asfortranarray(self._W.toarray()), check_finite=False, overwrite_a=True
        )
        # Round results to remove unwanted side effects
        self._W: npt.NDArray = np.asfortranarray(
            np.round(self._W, 10) + 0, dtype=np.complex64
        )
        eigs = np.round(eigs, 9) + 0
        eigs = -np.sqrt(eigs)
        eig_matrix: spmatrix = scs.diags(eigs, **spm_format)
        logger.info("Determine SMatrix V")
        self._V: npt.NDArray = Q.toarray() @ self._W @ scsl.inv(eig_matrix).toarray()
...
class SPMatrix(SMBase):
    """Scattering Matrix for highly sparse cases (>50% 0s) uniform layers"""
    ...
    def __init__(
        ...
        logger.info(f"Initializing SPMAtrix...")
        inv_er = scs.csc_matrix(inv(er, check_finite=False), dtype=np.complex64)
        inv_u0 = scs.csc_matrix(inv(u0, check_finite=False), dtype=np.complex64)
        logger.debug(
            f"""
...
"""
        ...
        self._W: spmatrix = P @ Q
        logger.debug(f"Omega2({self._W.shape}::{self._W.count_nonzero()})\n{self._W}")
        logger.info("Solve for eigenvalues")
        eigs, self._W = eig(self._W.toarray(), check_finite=False)
        self._W: spmatrix = scs.csc_matrix(self._W, dtype=np.complex64)
        # Round results to remove unwanted side effects
        eigs = np.round(eigs, 9) + 0
        eigs = -np.sqrt(eigs)
        eig_matrix: spmatrix = scs.diags(eigs, **spm_format)
        logger.info("Determine SPMatrix V")
        self._V: spmatrix = Q @ self._W @ scsl.inv(eig_matrix)
        logger.debug(
            f"""
Eig_Matrix ({eig_matrix.shape}):
{eig_matrix}
...
"""
        ...
        logger.info("Determine SPMatrix Pre-Elements")
        # Calculate the SMM elements
        inv_W: spmatrix = scsl.inv(self._W)
        inv_V: spmatrix = scsl.inv(self._V)
...
"""
        ...
        XB = X @ B
        XA = X @ A
        D = scsl.inv(A - XB @ inv_A @ XB)
        logger.info("Determine SPMatrix Elements")
        self._S11 = (D @ (XB @ inv_A @ XA - B)).toarray()
        self._S12 = (D @ X @ (A - B @ inv_A @ B)).toarray()
        self._S21 = self._S12
        self._S22 = self._S11
        logger.debug(self)
        logger.info(f"Finished Initializing SPMatrix...")
...
def r_t_fields(
    ...
    logger.debug(f"E_ref:[{e_ref_x} {e_ref_y} {rz}]\n E_Trn:[{e_trn_x} {e_trn_y} {tz}]")
...
def initialize_components(
    ...
    kz_ref_int = np.array(
        u_ref.conjugate() * e_ref.conjugate() - kx_mesh**2 - ky_mesh**2,
        dtype=np.complex64,
    )
    kz_trn_int = np.array(
        u_trn.conjugate() * e_trn.conjugate() - kx_mesh**2 - ky_mesh**2,
        dtype=np.complex64,
        order="F",
    )
