...
if city == 'Sines':   # Wh/Wpeak or Wh per day, average annual PV energy minus system losses
    EC_supply_pd = 4.324889045510265
elif city == 'Edmonton':
    EC_supply_pd = 3.1025502151038737
elif city =='Crystal Brook':
    EC_supply_pd = 3.978583890969629
else:
    print ('invalid city')
    sys.exit()
...
def solar_parameters_1dataset(L_days_hours, tilt, latitude, longitude, timezone, height, ar, L1_DNI, L1_DHI, L_Tair, L_wind, PV_efficiency, PV_system_losses, Tcoef_power, Tref):
    ...
    for i in range(0, len(L_days_hours)):
        ...
        DHI_correction = (math.radians(180) - tilt_r) / math.radians(180) 
...
def lists_per_day(days, L_days, *args):
    '''
    Total daily values throughout the set period (Wh/Wpeak per day --> Wh per day = suming the 24 values of W during a day
    '''
        ...
        for i_row in range(len(L_days)):
            if L_days[i_row] == d:
                for i_col in range(0, len(V_climate_pd)):
                    V_climate_pd[i_col] = max(V_climate_pd[i_col], L_climate[i_row][i_col])
        for i_col in range(0, len(V_climate_pd)):
...
def EC_calculations_1(EC_supply_pd, EC_production_pd, EC_consumption_pd, EC_water, H2_heating_value, L1T_gen):
    ...
    Varaibles and units
        EC_production_pd (volume of H2 produced per hour) = Nm3/day
        EC_consumption_pd (energy consumed by the EC per hour) = Wh/day
        EC_water (water consumed per Nm3 of hydrogen produced) = L/Nm3
        H2_heating_value (lower heating value of hydrogen, aka energy density) = MJ/kg
    ...
    List_pd = [EC_supply_pd, H2_volume_pd, H2_calorific_power_pd, H2_mass_pd, EC_water_pd]
    EC_supply_ph, H2_volume_ph, H2_calorific_power_ph, H2_mass_ph, EC_water_ph = [pd/24 for pd in List_pd]          # Wh/Wpeak or Wh per day, Nm3/Wpeak per day, mL/Wpeak per day
    EC_supply_To, H2_volume_To, H2_calorific_power_To, H2_mass_To, EC_water_To = [pd*len(days) for pd in List_pd]   # Wh/Wpeak or Wh per year, Nm3/Wpeak per year, mL/Wpeak per year
...
L_H2_mass_ph, L_H2_mass_pd = [H2_mass_ph]*len(L_hours), [H2_mass_ph]*len(days)
#                     0                 1                    2                3                       4              5
Lists_H2 = [L_grid_imports_ph, L_grid_exports_ph, L_battery_storage_ph, L_EC_supply_ph, L_H2_calorific_power_ph, L_H2_mass_ph]    
To_H2, Max_H2, Min_H2, Avr_H2, Std_H2 = calculate_statistics(*Lists_H2)                                                                                  # Total (Wh/Wpeak per set period), max, average and standard deviation (W/Wpeak)
...
L_irr = [Lt_GHI_pd, Lt_GTI_pd ,L1_GHI_pd, L1_GTI_pd]    # W/m2k to kW/m2
Lt_GHI_pd, Lt_GTI_pd ,L1_GHI_pd, L1_GTI_pd= [[round(value/1000,3) for value in L] for L in L_irr] 
   
L_EC_ph = [EC_supply_ph, H2_volume_ph, H2_calorific_power_ph, EC_water_ph]
EC_supply_ph, H2_volume_ph, H2_calorific_power_ph, EC_water_ph = [round(value,3) for value in L_EC_ph] 

L_EC_pd = [EC_supply_pd, H2_volume_pd, H2_calorific_power_pd, EC_water_pd, EC_supply_To, H2_volume_To, H2_calorific_power_To, EC_water_To]
EC_supply_pd, H2_volume_pd, H2_calorific_power_pd, EC_supply_To, EC_water_pd, H2_volume_To, H2_calorific_power_To, EC_water_To = [round(value,3) for value in L_EC_pd] 

L_H2_mass = [H2_mass_ph, H2_mass_pd, H2_mass_To]
H2_mass_ph, H2_mass_pd, H2_mass_To = [round(value*1000,3) for value in L_H2_mass]   # kg to g and round 

# Net flow

net_flow_pd = [round(PV + imp - EC - exp,2) for PV, imp, EC, exp in zip (L1T_gen_pd, L_grid_imports_pd, L_EC_supply_pd, L_grid_exports_pd)]    
...
print('battery capacity =',round(battery_capacity,3),'Wh/Wpeak')
print('')
...
print('kWh/m2/set period |  W/m2   |  W/m2')
print('')
...
print('')
print('Grid exports')
print('DS1:',round(To_H2[1],2),'|',Avr_H2[1],'|',Std_H2[1],'|',Max_H2[1])
print('')
print('EC OPERATION')
print('')
print('-  Total | Value per hour| value per day')
print('')
print('EC supply (Wh/Wpeak)')
print('DS1:',EC_supply_To,'|',EC_supply_ph,'|',EC_supply_pd)
print('')
print('H2 mass (g/Wpeak)')
print('DS1:',H2_mass_To,'|',H2_mass_ph,'|',H2_mass_pd)
print('')
print('H2 calorific value (Wh/Wpeak)')
print('DS1:',H2_calorific_power_To,'|',H2_calorific_power_ph,'|',H2_calorific_power_pd)
print('')
print('Water consumption (L/Wpeak)')
print('DS1:',EC_water_To,'|',EC_water_ph,'|',EC_water_pd)
print('')
print('')
print('- Net flow per day: PV + imp = EC + exp (should be 0)')
print(net_flow_pd)
